...
def core():
    global my_priv_key, my_pub_key, target_pub_key, CHUNK_SIZE
                ...
                if len(received_chunks) == expected_chunks:
                    ...
                    if my_priv_key:
                        ...
                        if full_command["status"] != 200:
                            send_msg("Failed to decrypt the message, there maybe a packet loss or a problem with the keys.")
                            received_chunks = {}
                            expected_chunks = None
                            gc.collect()
                            continue
                        ...
                        if full_command.startswith("target_chunk"):
                            ...
                            send_msg(f"set chunk size => {CHUNK_SIZE}")
                            received_chunks = {}
                            expected_chunks = None
                            gc.collect()
                            continue
                        ...
                        send_msg("Keys wasn't exchange or your key was lost, please initiate keys exchange to be able to run commands.")
                        received_chunks = {}
                        expected_chunks = None
                        gc.collect()
                        continue
